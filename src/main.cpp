...
#if defined(NDEBUG)
#error "COSMITTO cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "COSMITTO Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight <= 86400 && nHeight > 0) {
		...
		nSubsidy = 50 * COIN;
	} else if (nHeight > 408960 && nHeight <= 411840) { // 408961 => 1 COSMITTO FOR THE FIRST FEW HOURS OF POS AND LAST FEW HOURS OF POW
		nSubsidy = 1 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from COSMITTO
...
{
    RenameThread("COSMITTO-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // COSMITTO: We use certain sporks during IBD, so check to see if they are
